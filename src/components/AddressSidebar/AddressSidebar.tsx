...
import {
  ...
  CheckCircle2,
  ChevronDown,
} from "lucide-react"; // Update path as needed
...
export default function AddressSidebar({
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className={`fixed inset-0 z-60 bg-black/40 backdrop-blur-sm transition-opacity duration-300 ease-out ${
      ...
      {/* Sidebar Panel */}
      <aside
        className={`fixed top-0 right-0 z-70 h-full w-full max-w-md bg-background shadow-2xl transform transition-transform duration-300 ease-out flex flex-col ${
              ...
              {addresses.map((addr) => {
                ...
                return (
                  <div
                    ...
                    className={`relative p-4 rounded-xl border-2 transition-all cursor-pointer select-none hover:shadow-sm ${
                      addr.isDefault
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3 pt-3 border-t border-border/50">
                      ...
                      <button
                        onClick={(e) =>
                          addr._id && toggleAddressExpand(addr._id, e)
                        }
...
function AddressForm({ initialData, onSubmit, onCancel }: AddressFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="block text-xs font-medium text-admin-text/70 mb-1">
          Full Name
        </label>
        <input
          required
