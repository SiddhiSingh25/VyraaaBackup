...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  const wishlistIds: string[] = useSelector((state: any) => state.wishlist?.items ?? []);
  ...
  const handleWishlist = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const url = wasWished
      ? apiUrls.WishList.remove + product._id // ✅ adjust to your actual remove endpoint
      : apiUrls.WishList.add + product._id;

    getQuery({
      url,
      onSuccess: () => {
        dispatch(wasWished ? removeFromWishlist(product._id) : addToWishlist(product._id));
        toast("success", wasWished ? "Removed from wishlist" : "Added to wishlist!");
      },
      onFail: () => {
        toast("error", wasWished ? "Failed to remove from wishlist." : "Failed to add to wishlist.");
      },
