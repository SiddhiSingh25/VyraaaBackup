...
export default function AdminLayout() {
  ...
  return (
    <div className="relative flex min-h-screen bg-background font-admin-text text-[13.5px] sm:text-sm md:text-[15px]">
      {isMobileSidebarOpen && (
        <button
          type="button"
          aria-label="Close sidebar overlay"
      ...
      <aside
        ...
        className={`
          ...
          ${isMobileSidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}
          ${isAsideExpanded ? "lg:w-67.5" : "lg:w-19"}
          w-[min(18rem,86vw)] sm:w-72
        `}
      >
        <div className="border-b border-border/70 px-3.5 py-3.5 sm:px-4 sm:py-4">
          <div
            className={`flex items-center gap-2.5 sm:gap-3 ${isAsideExpanded ? "justify-between" : "justify-center"}`}
          >
            <button
              type="button"
              // onClick={() => navigate("/admin/dashboard")}
              className={`
                flex w-full min-w-0 items-center rounded-xl transition-all duration-200
                ${isAsideExpanded ? "justify-start gap-2.5 sm:gap-3" : "justify-center"}
              `}
            >
              <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl bg-linear-to-br from-primary to-primary/70 shadow-lg shadow-primary/25 sm:h-10 sm:w-10">
                <Bolt className="h-4.5 w-4.5 text-white sm:h-5 sm:w-5" />
              </span>
              ...
              <span
                ...
                `}
              >
                <span className="block truncate text-base font-bold tracking-wide sm:text-lg">
                  Admin
                </span>
            ...
            <div
              className={`flex items-center gap-1.5 sm:gap-2 ${isAsideExpanded || isMobileSidebarOpen ? "flex" : "hidden"}`}
            >
              <button
                type="button"
                onClick={() => setIsLocked((prev) => !prev)}
                className={`
                  hidden h-8 w-8 items-center justify-center rounded-xl border
                  border-border/70 bg-background/70 text-muted transition-all
                  duration-200 hover:bg-card hover:text-primary sm:h-9 sm:w-9 lg:flex
                  ${isAsideExpanded ? "opacity-100" : "pointer-events-none opacity-0"}
                `}
                title={isLocked ? "Unlock sidebar" : "Lock sidebar"}
              >
              ...
              <button
                ...
                aria-label="Close sidebar"
                onClick={() => setIsMobileSidebarOpen(false)}
                className="flex h-8 w-8 items-center justify-center rounded-xl border border-border/70 bg-background/70 text-muted transition-all duration-200 hover:bg-card hover:text-primary sm:h-9 sm:w-9 lg:hidden"
              >
        ...
        <div className="px-2.5 py-2.5 sm:px-3 sm:py-3">
          <button
            ...
            className={`
              group flex w-full items-center rounded-2xl bg-linear-to-r
              from-primary via-primary/90 to-primary/75 px-2.5 py-2 text-[11px]
              font-semibold uppercase tracking-[0.14em] text-white shadow-lg
              shadow-primary/20 transition-all duration-300 hover:-translate-y-0.5
              hover:shadow-primary/30 sm:px-3 sm:py-2.5 sm:text-xs sm:tracking-[0.18em]
              ${isAsideExpanded ? "lg:justify-start lg:gap-3" : "lg:justify-center"}
            `}
          >
            <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-white/20 transition-transform duration-300 group-hover:rotate-90 sm:h-7 sm:w-7">
        ...
        <nav className="flex-1 min-h-0 px-2.5 pb-2.5 sm:px-3 sm:pb-3">
          <div className="h-full overflow-y-auto pr-1 scrollbar-none">
            <div className="space-y-1">
              {navItems.map((item: any) => {
                const Icon = item.icon;
                  ...
                  return (
                    <div key={item.label} className="space-y-1">
                      <button
                        ...
                        title={item.label}
                        className={`
                          group flex w-full items-center rounded-2xl px-2.5 py-2
                          text-[13px] transition-all duration-200 sm:px-3 sm:py-2.5 sm:text-sm
                          ${isAsideExpanded ? "lg:justify-between" : "lg:justify-center"}
                          ${
                            isParentActive
                              ? "bg-primary/10 text-primary"
                      ...
                      >
                        ...
                        >
                          <span
                            ...
                            `}
                          >
                            <Icon className="h-3.5 w-3.5 sm:h-[17px] sm:w-[17px]" />
                          </span>
                        ...
                        >
                          {isOpen ? (
                            <ChevronDown className="h-3.5 w-3.5 text-primary sm:h-[17px] sm:w-[17px]" />
                          ) : (
                      ...
                      {isOpen && (
                        <div
                          ...
                          `}
                        >
                          <div className="ml-4 rounded-2xl border border-border/60 bg-background/45 p-1.5 backdrop-blur-xl sm:ml-5 sm:p-2">
                            <div className="relative space-y-1 border-l border-primary/20 pl-2.5 sm:pl-3">
                              {item.items.length === 0 ? (
                                <p className="px-2.5 py-2 text-[11px] text-muted sm:px-3 sm:text-xs">
                                  No categories found
                                </p>
                              ) : (
                                item.items.map((sub: NavSubItem) => {
                                  ...
                                  return (
                                    <button
                                      ...
                                      className={`
                                        group relative flex w-full items-center gap-2 rounded-xl
                                        px-2.5 py-1.5 text-left text-[11px] transition-all
                                        duration-200 sm:px-3 sm:py-2 sm:text-xs
                                        ${
                                          isSubActive
                                            ? "bg-primary text-white shadow-sm shadow-primary/20"
                                            : "text-muted hover:bg-card hover:"
                                    ...
                                    >
                                      <span
                                        className={`
                                          absolute -left-3.75 h-2 w-2 rounded-full border-2
                                          sm:-left-4.25 sm:h-2.5 sm:w-2.5
                                          ${
                                            isSubActive
                                              ? "border-primary bg-white"
                                              : "border-primary/40 bg-background"
                                      ...
                                      <ChevronRight
                                        className={`
                                          ...
                                          group-hover:translate-x-0.5 sm:h-[13px] sm:w-[13px]
                                          ${isSubActive ? "text-white" : "text-primary/70"}
                                        `}
                                      />
                                    </button>
                ...
                return (
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `
                        group flex items-center rounded-2xl px-2.5 py-2 text-[13px]
                        transition-all duration-200 sm:px-3 sm:py-2.5 sm:text-sm
                        ${isAsideExpanded ? "lg:gap-3" : "lg:justify-center"}
                        gap-2.5 sm:gap-3
                        ${
                          isActive
        ...
        <div className="border-t border-border/70 p-2.5 sm:p-3">
          <button
            ...
            onClick={handleLogout}
            className={`
              flex w-full items-center rounded-2xl px-2.5 py-2 text-[13px]
              font-medium text-primary transition-all duration-200 hover:bg-primary/10
          ...
          >
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary sm:h-9 sm:w-9">
              <LogOut className="h-3.5 w-3.5 sm:h-[17px] sm:w-[17px]" />
            </span>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header
          className="
            sticky top-0 z-20 border-b border-border/60
            bg-background/55 px-3.5 py-2.5 backdrop-blur-2xl
            supports-[backdrop-filter]:bg-background/45
            sm:px-5 sm:py-3 lg:hidden
          "
        >
          <div className="flex items-center justify-between gap-2.5 sm:gap-3">
            <div className="flex min-w-0 items-center gap-2.5 sm:gap-3">
              <button
                ...
                aria-label="Open sidebar"
                onClick={() => setIsMobileSidebarOpen(true)}
                className="flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl border border-border/70 bg-surface/70 text-muted shadow-sm backdrop-blur-xl transition-all duration-200 hover:bg-card hover:text-primary sm:h-10 sm:w-10"
              >
              ...
              <div className="min-w-0">
                ...
                <p className="hidden text-[11px] text-muted sm:block sm:text-xs">
                  Manage your store data, products, and settings
                </p>
              </div>
            </div>
            ...
            <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
              <button
                ...
                onClick={handleLogout}
                aria-label="Logout"
                className="flex h-9 w-9 items-center justify-center rounded-xl bg-primary/10 text-primary transition-all duration-200 hover:bg-primary hover:text-white sm:h-10 sm:w-10"
              >
                <LogOut className="h-3.5 w-3.5 sm:h-[17px] sm:w-[17px]" />
              </button>
            </div>
        ...
        <main className="flex-1 overflow-y-auto overflow-x-hidden bg-background">
          <div className="mx-auto w-full max-w-[1920px] px-3 py-3 sm:px-4 sm:py-4 md:px-6 md:py-5 lg:px-8 lg:py-6">
            <Outlet />
          </div>
        </main>
      </div>
