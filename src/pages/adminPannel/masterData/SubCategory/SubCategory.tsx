...
export default function SubCategory() {
  ...
  const fetchCategories = () => {
    getQuery({ url: apiUrls.Category.getAll, onSuccess: (res: any) => {
      const data = res?.data || [];
  ...
  const fetchSubcategories = () => {
    getQuery({ url: apiUrls.SubCategory.getAll, onSuccess: (res: any) => {
      const data = res?.data || [];
      setItems(data.map(mapApi).map((it: SubCategory, idx: number) => ({
        ...it,
        srNo: idx + 1,
        categoryName: categories.find((c) => c.id === it.category)?.name || '',
      })));
    }});
  };

  useEffect(() => { fetchCategories(); }, []);
  useEffect(() => { fetchSubcategories(); }, [categories]);

  const openAdd = () => { setModalMode('add'); setActiveItem(null); setIsFormOpen(true); };
  const openEdit = (it: SubCategory) => { setModalMode('edit'); setActiveItem(it); setIsFormOpen(true); };
  const closeForm = () => setIsFormOpen(false);

  const handleSubmit = async (values: SubCategoryFormValues) => {
    if (modalMode === 'add') {
      await postQuery({ url: apiUrls.SubCategory.add, postData: { category: values.category, subCategory: values.subCategory }, onSuccess: (res: any) => {
        const newItem = res?.data; if (!newItem) return;
        setItems((prev) => prev.concat({ ...mapApi(newItem), srNo: prev.length + 1, categoryName: categories.find(c => c.id === newItem.category)?.name || '' }));
        setIsFormOpen(false);
      }});
    } else if (activeItem) {
      await putQuery({ url: apiUrls.SubCategory.update, putData: { id: activeItem.id, subCategory: values.subCategory }, onSuccess: (res: any) => {
        const updated = res?.data; if (!updated) return;
        setItems((prev) => prev.map((p) => p.id === updated._id ? { ...p, subCategory: updated.subCategory } : p));
        setIsFormOpen(false);
      }});
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    await deleteQuery({ url: apiUrls.SubCategory.delete, deleteData: { id: pendingDelete.id }, onSuccess: () => {
  ...
  return (
    <div className="h-screen bg-slate-50 font-sans text-slate-900">
      ...
      <SubCategoryFormModal isOpen={isFormOpen} mode={modalMode} categories={categories} initialData={activeItem ?? null} onClose={closeForm} onSubmit={handleSubmit} />

      <ConfirmDialog isOpen={Boolean(pendingDelete)} title="Delete subcategory?" description={pendingDelete ? `This will permanently remove "${pendingDelete.subCategory}".` : ''} onConfirm={confirmDelete} onCancel={() => setPendingDelete(null)} />
    </div>
  );
}
