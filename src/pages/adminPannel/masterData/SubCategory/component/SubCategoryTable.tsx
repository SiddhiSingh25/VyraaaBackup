...
const COLUMNS = ['Sr. No.', 'Subcategory', 'Category', 'Actions'];

export default function SubCategoryTable({ items, onEdit, onDelete }: Props) {
  return (
    // Match CategoryTable visual layout
          ...
          <tbody>
            ...
            {items.map((it, idx) => (
              ...
              >
                ...
                <td className="whitespace-nowrap px-4 py-3.5 text-sm text-heading font-medium">{it.subCategory}</td>
                <td className="whitespace-nowrap px-4 py-3.5 text-sm text-heading">{it.categoryName ?? it.category}</td>
                <td className="whitespace-nowrap px-4 py-3.5">
                  <div className="flex items-center justify-end gap-2.5 pr-2">
                    <button
                      onClick={() => onEdit(it)}
                      aria-label={`Edit ${it.subCategory}`}
                    ...
                    <button
                      onClick={() => onDelete(it)}
                      aria-label={`Delete ${it.subCategory}`}
