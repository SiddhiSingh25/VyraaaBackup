...
const mapApi = (item: any): ColorFamily => ({
  id: item._id,
  srNo: 0,
  colorFamily: item.colorFamily || item.colorFamily,
});
...
export default function ColorFamilyPage() {
    ...
    getQuery({
      ...
      onSuccess: (res: any) => {
        ...
        setItems(
          data
            .map(mapApi)
            .map((it: ColorFamily, idx: number) => ({ ...it, srNo: idx + 1 })),
        );
      },
    });
  };
  ...
  const handleSubmit = async (values: ColorFamilyFormValues) => {
    if (modalMode === "add") {
      await postQuery({
        ...
        postData: { colorFamily: values.colorFamily },
        onSuccess: (res: any) => {
          const newItem = res?.data;
          if (!newItem) return;
          setItems((prev) =>
            prev.concat({ ...mapApi(newItem), srNo: prev.length + 1 }),
          );
          setIsFormOpen(false);
        },
      });
      ...
      await putQuery({
        url: apiUrls.ColorFamily.update,
        putData: { id: activeItem.id, colorFamily: values.colorFamily },
        onSuccess: (res: any) => {
          ...
          setItems((prev) =>
            prev.map((p) =>
              p.id === updated._id
                ? { ...p, colorFamily: updated.colorFamily }
                : p,
            ),
          );
          setIsFormOpen(false);
  ...
  const confirmDelete = async () => {
    ...
    await deleteQuery({
      url: apiUrls.ColorFamily.delete,
  ...
  return (
    <div className="h-screen bg-slate-50 font-admin-text text-slate-900">
      <div className="mx-auto max-w-7xl px-4 sm:px-4 lg:px-4 py-4">
        ...
        {loading && (
          <PageLoader loading={loading} text="Loading Color Families..." />
        )}
      ...
      <ConfirmDialog
        ...
        description={
          pendingDelete
            ? `This will permanently remove "${pendingDelete.colorFamily}".`
            : ""
        }
