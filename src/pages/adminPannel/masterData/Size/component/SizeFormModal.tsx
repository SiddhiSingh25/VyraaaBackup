...
export default function SizeFormModal({
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={mode === "add" ? "Add Size" : "Edit Size"}
      description={mode === "add" ? "Create a new size value." : "Update size."}
      footer={
        <>
      ...
      >
        ...
        <div className="sm:col-span-2">
          ...
          <input
            ...
            placeholder="e.g. XS"
            className={`w-full rounded-lg border px-3.5 h-10 text-sm text-dark-900 ${errors.size ? "border-red-300" : "border-dark-200"}`}
          />
          {errors.size && (
            <p className="mt-1 text-xs font-medium text-red-500">
              {errors.size}
