        ...
        <table className="w-full min-w-205 border-collapse text-left relative">
          <thead className="bg-surface sticky top-0 z-10">
            <tr className="border-b-2 border-primary shadow-sm">
              {COLUMNS.map((col, i) => (
                <th
                  key={col}
                  className={`whitespace-nowrap py-3.5 px-4 text-sm font-medium text-primary bg-surface ${col === "Actions" ? "text-right pr-6" : "text-left"} ${i === 0 ? "text-center" : ""} ${i !== COLUMNS.length - 1 ? "border-r border-border/60" : ""}`}
                >
                  {col}
                </th>
              ))}
          ...
          <tbody>
            ...
            {items.map((it, idx) => (
              <tr
                key={it.id}
                className={`border-b border-border/60 last:border-b-0 hover:bg-card/40 transition-colors ${idx % 2 === 0 ? "bg-surface" : "bg-background"}`}
              >
