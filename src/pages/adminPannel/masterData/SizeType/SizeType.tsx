...
const mapApi = (item: any): SizeTypeItem => ({
  id: item._id,
  srNo: 0,
  sizeType: item.sizeType || item.sizeType,
});
...
export default function SizeTypePage() {
    ...
    getQuery({
      ...
      onSuccess: (res: any) => {
        ...
        setItems(
          data
            .map(mapApi)
            .map((it: SizeTypeItem, idx: number) => ({ ...it, srNo: idx + 1 })),
        );
      },
    });
  };
  ...
  const openEdit = (it: SizeTypeItem) => {
    setModalMode("edit");
    setActiveItem(it);
    setIsFormOpen(true);
  };
  ...
  const handleSubmit = async (values: SizeTypeFormValues) => {
    if (modalMode === "add") {
      await postQuery({
        ...
        postData: { sizeType: values.sizeType },
        onSuccess: (res: any) => {
      ...
      await putQuery({
        url: apiUrls.SizeType.update,
        putData: { id: activeItem.id, sizeType: values.sizeType },
        onSuccess: (res: any) => {
          const updated = res?.data;
          if (!updated) return;
          setItems((prev) =>
            prev.map((p) =>
              p.id === updated._id ? { ...p, sizeType: updated.sizeType } : p,
            ),
          );
          setIsFormOpen(false);
        },
  ...
  return (
    <div className="h-screen bg-slate-50 font-admin-text text-slate-900">
      <div className="mx-auto max-w-7xl px-4 sm:px-4 lg:px-4 py-4">
        ...
        {loading && <PageLoader loading={loading} text="Loading Sizetype..." />}
      ...
      <ConfirmDialog
        ...
        description={
          pendingDelete
            ? `This will permanently remove "${pendingDelete.sizeType}".`
            : ""
        }
