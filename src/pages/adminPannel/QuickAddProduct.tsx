...
const schema: yup.ObjectSchema<QuickAddValues> = yup.object({
    ...
    attributes: yup.array().of(
        yup.object().shape({ property: yup.string().required(), value: yup.string().required() })
    ).default([]),
    ...
    variants: yup.array().of(
        yup.object({
            ...
            price: yup.number().typeError("Must be a number").required("Price is required").min(1, "Price must be > 0"),
            stock: yup.number().typeError("Must be a number").required("Stock is required").min(0, "Invalid stock"),
    ...
    ).min(1, "At least one variant is required").default([]),

    // Media
    images: yup.array().of(yup.string().required()).required("At least one image is required").min(1, "At least one image is required").max(5, "Max 5 images for Quick Add"),
...
const QuickAddProduct = () => {
    const { register, handleSubmit, watch, setValue, formState: { errors }, reset } = useForm<QuickAddValues>({
    ...
    const [draftVariant, setDraftVariant] = useState({ size: "", price: "", stock: "", sku: "", discountPrice: "", isAvailable: true, isFewLeft: false });
    ...
    const addVariant = () => {
        if (selectedSizeType && draftVariant.size && draftVariant.price && draftVariant.stock && draftVariant.sku) {
            const newVariant = {
                size: draftVariant.size,
                price: Number(draftVariant.price),
                stock: Number(draftVariant.stock),
                sku: draftVariant.sku,
                discountPrice: draftVariant.discountPrice ? Number(draftVariant.discountPrice) : undefined,
                isAvailable: draftVariant.isAvailable,
                isFewLeft: draftVariant.isFewLeft,
            };
            setValue("variants", [...variants, newVariant] as any);
            setDraftVariant({ size: "", price: "", stock: "", sku: "", discountPrice: "", isAvailable: true, isFewLeft: false });
            ...
            <main className="flex-1 overflow-y-auto">
                <form onSubmit={handleSubmit(onSubmit)} className="mx-auto flex h-full max-w-6xl flex-col gap-8 px-4 py-8 sm:px-6 lg:px-8">
                    ...
                    <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between bg-surface p-6 rounded-xl border border-border shadow-sm">
                        <div>
                            <h2 className="text-3xl font-semibold text-heading font-heading tracking-wide">Quick Add Product</h2>
                            <p className="text-sm text-muted mt-1">Streamlined entry for a single SKU mapping to your master taxonomy.</p>
                        </div>
                        <div className="flex flex-wrap gap-4">
                            <Button type="button" variant="secondary" onClick={() => reset()}>Clear</Button>
                            <Button type="submit" variant="primary">Publish SKU</Button>
                        </div>
                    </div>
                        ...
                        <div className="flex flex-col gap-8">
                            ...
                            <section className="rounded-xl border border-border bg-surface p-6 shadow-sm">
                                <div className="flex items-center gap-3 border-b border-border pb-4 mb-6">
                                    ...
                                    <h3 className="text-lg text-heading font-heading font-semibold">Taxonomy Mapping</h3>
                                </div>
                                ...
                                <div className="grid gap-5 sm:grid-cols-3">
                                    <Select
                                        ...
                                        options={Object.keys(MASTER_TAXONOMY).map(c => ({ label: c, value: c }))}
                                    />
                                    <Select
                                        label="Subcategory"
                                        required
                                        ...
                                        options={selectedCategory ? Object.keys(MASTER_TAXONOMY[selectedCategory] || {}).map(sc => ({ label: sc, value: sc })) : []}
                                    />
                                    <Select
                                        ...
                                        options={selectedCategory && selectedSubcategory ? MASTER_TAXONOMY[selectedCategory][selectedSubcategory].map(t => ({ label: t, value: t })) : []}
                                    />
                                </div>
                            </section>
                            ...
                            <section className="rounded-xl border border-border bg-surface p-6 shadow-sm">
                                <div className="flex items-center gap-3 border-b border-border pb-4 mb-6">
                                    ...
                                    <h3 className="text-lg text-heading font-heading font-semibold">Core Information</h3>
                                </div>
                                ...
                                <div className="flex flex-col gap-5">
                                    <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
                                        <Select
                                            ...
                                            options={Object.keys(COLOR_FAMILY_DATA).map(cf => ({ label: cf, value: cf }))}
                                        />
                                        <Select
                                            ...
                                            options={selectedColorFamily ? COLOR_FAMILY_DATA[selectedColorFamily].map(c => ({ label: c, value: c })) : []}
                                        />
                                        <Select
                                            label="Size Type"
                                            required
                                            ...
                                            options={Object.keys(SIZE_TYPE_DATA).map(st => ({ label: st, value: st }))}
                                        />
                                    </div>
                                    <Input
                                        label="Product Name"
                            ...
                            <section className="rounded-xl border border-border bg-surface p-6 shadow-sm">
                                <div className="flex items-center gap-3 border-b border-border pb-4 mb-6">
                                    ...
                                    <h3 className="text-lg text-heading font-heading font-semibold">Variants & Pricing</h3>
                                </div>

                                <div className="mb-5 flex flex-wrap items-center justify-between gap-3 border-b border-border pb-4">
                                    <div>
                                        <p className="text-sm font-medium text-heading">Add SKU variants</p>
                                        <p className="text-sm text-muted">Create one or more size-based entries for this product.</p>
                                    </div>
                                        ...
                                        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
                                            <div className="w-full">
                                                ...
                                                >
                                                    ...
                                                    {selectedSizeType ? SIZE_TYPE_DATA[selectedSizeType].map(s => <option key={s} value={s}>{s}</option>) : null}
                                                </select>
                                            </div>
                                            <div className="w-full">
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => setDraftVariant({ ...draftVariant, price: e.target.value })}
                                                    placeholder="0.00"
                                            ...
                                            <div className="w-full">
                                                <label className="mb-1.5 block text-[11px] font-bold uppercase tracking-[0.1em] text-muted">Stock Qty</label>
                                                <input
                                                    type="number"
                                            ...
                                            <div className="w-full">
                                                <label className="mb-1.5 block text-[11px] font-bold uppercase tracking-[0.1em] text-muted">SKU Code</label>
                                                <input
                                                    type="text"
                                            ...
                                            <div className="flex flex-1 flex-wrap items-center gap-4">
                                                <div className="min-w-[140px] flex-1">
                                                    ...
                                                    <input
                                                        ...
                                                        onChange={(e) => setDraftVariant({ ...draftVariant, discountPrice: e.target.value })}
                                            ...
                                            <tbody className="divide-y divide-border">
                                                {variants.map((variant: any, index: number) => (
                                                    <tr key={index} className="hover:bg-card/40 transition-colors text-body">
                                                        <td className="px-5 py-4 font-medium">{variant.size}</td>
                                                        <td className="px-5 py-4">₹{variant.price}</td>
                                                        <td className="px-5 py-4">{variant.stock}</td>
                                                        <td className="px-5 py-4">{variant.sku}</td>
                                                        <td className="px-5 py-4 text-muted">{variant.discountPrice ? `₹${variant.discountPrice}` : '-'}</td>
                                                        <td className="px-5 py-4">
                                                            <div className="flex items-center gap-2">
                                                                <span className={`h-2.5 w-2.5 rounded-full ${variant.isAvailable ? 'bg-success' : 'bg-error'}`}></span>
                                                                {variant.isFewLeft && <span className="rounded-md bg-warning/20 px-2 py-0.5 text-[10px] font-bold text-warning uppercase tracking-wider">Few Left</span>}
                                                            </div>
                                                        </td>
                                                        <td className="px-5 py-4 text-right">
                                                            <button type="button" onClick={() => removeVariant(index)} className="text-muted hover:text-error transition">
                            ...
                            <section className="rounded-xl border border-border bg-surface p-6 shadow-sm">
                                <div className="flex items-center justify-between border-b border-border pb-4 mb-6">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <h3 className="text-lg text-heading font-heading font-semibold">Attributes</h3>
                                    </div>
                                </div>
                                ...
                                <div className="mb-5 flex flex-col gap-4 rounded-lg border border-primary-light bg-card p-4 sm:flex-row sm:items-end shadow-inner">
                                    ...
                                    <Button type="button" variant="primary" onClick={addAttribute} disabled={!draftAttr.property || !draftAttr.value} className="px-5 py-2">
                                        Add
                                    </Button>
                                </div>
                                                ...
                                                {attributes.map((attr, index) => (
                                                    <tr key={index} className="hover:bg-card/40 transition">
                                                        ...
                                                        <td className="px-4 py-3 text-right">
                                                            <button type="button" onClick={() => removeAttribute(index)} className="text-muted hover:text-error transition">
                                                                <MdOutlineDeleteSweep size={20} />
                                                            </button>
                                                        </td>
                                                    </tr>
                                ...
                                <div className="group relative flex aspect-[4/5] w-full items-center justify-center overflow-hidden rounded-xl border border-border bg-card transition hover:border-primary-light">
                                    {images.length > 0 ? (
                                        <>
                                            ...
                                            <button type="button" onClick={() => removeImage(0)} className="absolute right-3 top-3 z-10 rounded-full bg-background/90 p-2 text-error shadow-sm hover:bg-error hover:text-white transition">
                                                <RiCloseLine size={16} />
                                            </button>
                                        </>
                                    ) : (
                                ...
                                <div className="mt-4 grid grid-cols-4 gap-2">
                                    {images.slice(1).map((img, idx) => (
                                        <div key={idx} className="group relative aspect-square overflow-hidden rounded-lg border border-border bg-card">
                                            ...
                                            <button type="button" onClick={() => removeImage(idx + 1)} className="absolute inset-0 flex items-center justify-center bg-dark/50 text-white opacity-0 transition-opacity group-hover:opacity-100 backdrop-blur-sm">
                                                <RiCloseLine size={20} />
                                            </button>
                                        </div>
                                    ))}
